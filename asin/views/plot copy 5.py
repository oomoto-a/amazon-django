...
def get_svg(request, id, fbm, fba, cart):
    ...
    if keepaInfo.lowest_history_fbm and fbm == 1:
        csv = str(keepaInfo.lowest_history_fbm).split(",")
        product_data.update(parse_csv(csv, "FBM", 3, "o", cmap(0)))
        keys.append("FBM")

    
    if keepaInfo.lowest_history_fba and fba == 1:
        csv = str(keepaInfo.lowest_history_fba).split(",")
        product_data.update(parse_csv(csv, "FBA", 2, "s", cmap(2)))
        keys.append("FBA")
     
    if keepaInfo.buy_box_history and cart == 1:
        csv = str(keepaInfo.buy_box_history).split(",")
        product_data.update(parse_csv(csv, "カート", 3, "v", cmap(4)))
        keys.append("カート")
